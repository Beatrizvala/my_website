...
```{r, read_gss_data, cache=TRUE}
snap_insta <- gss %>% 
  mutate(Yes=c("snapchat","instagrm"), No=c())
...
```{r trump_margins,echo=FALSE, out.width="100%"}
Trump_net_approval <- approval_polllist %>%
  filter(subgroup == "Voters") %>%
  mutate(Date = mdy(enddate),
         Year = year(Date),
         Week = week(Date)) %>%
  group_by(Year, Week) %>%
  summarise(Average_net_approval = mean(adjusted_approve - adjusted_disapprove),
            Stdv = sd(adjusted_approve - adjusted_disapprove),
            Critical_value = qt(0.95, n() - 1),
            SE = Stdv / sqrt(n()),
            Margin_error = Critical_value * SE,
            Low = Average_net_approval - Margin_error,
            High = Average_net_approval + Margin_error)

  ggplot(data = Trump_net_approval, aes(Week, Average_net_approval)) +
...
The confidence interval in week 34 is wider than in week 15, which means that Trump's approval rating had a bigger dispersion from 17-23 August, than from 6-12 April. This shift may have occurred due to the way Trump has handled the Coronavirus pandemic, and also George Floyd's death, that led to several Black Lives Matter Movement's protests.
